...
`http_req_failed` tự động đánh dấu các mã phản hồi HTTP từ 200 đến 399 là thành công. Điều này có nghĩa là các mã phản hồi HTTP 4xx và HTTP 5xx được k6 coi là lỗi theo mặc định. (Lưu ý: Hành vi này có thể được thay đổi bằng cách sử dụng [`setResponseCallback`](https://grafana.com/docs/k6/latest/javascript-api/k6-http/set-response-callback/).)
